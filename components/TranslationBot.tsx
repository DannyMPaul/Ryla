...
const TranslationBot = () => {
  ...
  const fallbackTranslate = (text: string, isEnglishToFrench: boolean) => {
    ...
    if (isEnglishToFrench) {
      return fallbackTranslations[lowerText as keyof typeof fallbackTranslations] || null;
    } else {
      const entry = Object.entries(fallbackTranslations).find(([_, french]) => french.toLowerCase() === lowerText);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (isOnline) {
        ...
        response = `${isEnglishToFrench ? 'French' : 'English'} translation: "${translated}"`;
      } else {
        const fallbackResult = fallbackTranslate(userQuery, isEnglishToFrench);
        if (fallbackResult) {
          response = `${isEnglishToFrench ? 'French' : 'English'} translation: "${fallbackResult}"`;
        } else {
          response = "I'm sorry, I don't know that word yet. Please try another word or phrase.";
        }
      }
