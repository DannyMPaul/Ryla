...
export default function VoiceTranslatorScreen() {
  ...
  const checkLibreTranslateAvailability = async () => {
    console.log(
      `Attempting to connect to LibreTranslate at: ${LIBRE_TRANSLATE_API_URL}`
    );
    try {
      ...
      if (response.status === 200) {
        ...
        console.warn(
          "LibreTranslate returned unexpected status:",
          response.status
        );
      ...
      try {
        ...
        console.log(
          "Server is running but API endpoint may be wrong. Status:",
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const languagePhrases =
          phrases[sourceLanguage as keyof typeof phrases] || defaultPhrases;
        resolve(
          languagePhrases[Math.floor(Math.random() * languagePhrases.length)]
        );
      }, 1000);
    });
  };
  ...
  const translateText = async (
    ...
    try {
      ...
      const response = await axios.post(
        `${LIBRE_TRANSLATE_API_URL}/translate`,
        {
          q: text,
          source: from,
          target: to,
  ...
  const fallbackTranslation = (
    ...
    return `[Translated from ${
      LANGUAGES.find((l) => l.code === from)?.name
    } to ${LANGUAGES.find((l) => l.code === to)?.name}]: ${text}`;
  };
  ...
  const handleTextInput = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Translation Error",
        "Failed to translate your text. Please try again."
      );
    } finally {
      setIsTranslating(false);
    }
        ...
        {!libreTranslateAvailable && (
          <View style={styles.offlineNotice}>
            ...
            <Text style={styles.offlineText}>
              Offline Mode - Limited Translations
            </Text>
          </View>
        )}
